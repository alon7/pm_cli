...
// Truncates string to givenn max length, and inserts ellipsis into
...
func TruncateString(str string, maxRunes int) string {
	...
	// Input string is also returend if max length is less than 9 which is the minimal supported length
	...
	for leftOffset, char := range str {
		...
		// Start skipping chars when the left an dright offsets are equal
		// Or in the case where we wont be able to do an event split: when the left is larger than the right offset
...
// Retruns a padding function, that pads input to the longest string in items
