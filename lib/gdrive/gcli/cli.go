...
func List(d *Drive, query, titleFilter string, maxResults int, sharedStatus, noHeader, includeDocs, sizeInBytes bool) error {
	caller := d.Files.List()
	queryList := []string{}

	//Set the drive query maximum results
	if maxResults > 0 {
	...
	for _, f := range files {
		...
		items = append(items, map[string]string{
			"Id":      f.Id,
			"Title":   gutil.TruncateString(f.Title, 40),
			"Size":    gutil.FileSizeFormat(f.FileSize, sizeInBytes),
			"Created": gutil.ISODateToLocal(f.CreatedDate),
		})
	}
	...
	gutil.PrintColumns(items, columnOrder, 3, noHeader)
	return nil
}

// Adds the key-value-pair 'Shared: True/False' to the map
func addSharedStatus(d *Drive, items []map[string]string) {
	...
	checkStatus := func(item map[string]string) {
		...
		item["Shared"] = gutil.FormatBool(shared)

		// Decrement the active queu and notify that we are done
		<-active
		done <- true
	}
...
func Info(d *Drive, fileId string, sizeInBytes bool) error {
...
func printInfo(d *Drive, f *drive.File, sizeInBytes bool) {
	fields := map[string]string{
		...
		"Size":        gutil.FileSizeFormat(f.FileSize, sizeInBytes),
		"Created":     gutil.ISODateToLocal(f.CreatedDate),
		"Modified":    gutil.ISODateToLocal(f.ModifiedDate),
		"Owner":       strings.Join(f.OwnerNames, ", "),
		"Md5sum":      f.Md5Checksum,
		"Shared":      gutil.FormatBool(isShared(d, f.Id)),
		"Parents":     gutil.ParentList(f.Parents),
	}
	...
	gutil.Print(fields, order)
}

// Create folder in drive
func Folder(d *Drive, title string, parentId string, share bool) error {
...
func makeFolder(d *Drive, title string, parentId string, share bool) (*drive.File, error) {
	// File instance
	...
	if err != nil {
		return nil, fmt.Errorf("An error ocurred creating the folder: %v\n", err)
	}
	// Share the folder if the share flag was provided
	if share {
		Share(d, info.Id)
	}
	return info, err
...
func UploadStdin(d *Drive, input io.ReadCloser, title string, parentId string, share bool, mimeType string, convert bool) error {
	// File instance
	...
	getRate := gutil.MeasureTransferRate()
	...
	if err != nil {
		return fmt.Errorf("An error occurred uploading the documnet: %v\n", err)
	}
	...
	fmt.Printf("Uploaded '%s' at %s, total %s\n", info.Title, getRate(bytes), gutil.FileSizeFormat(bytes, false))
...
func Upload(d *Drive, input *os.File, title string, parentId string, share bool, mimeType string, convert bool) error {
	// Grab file info
...
func uploadDirectory(d *Drive, input *os.File, inputInfo os.FileInfo, title string, parentId string, share bool, mimeType string, convert bool) error {
	// Create folder
...
func uploadFile(d *Drive, input *os.File, inputInfo os.FileInfo, title string, parentId string, share bool, mimeType string, convert bool) error {
	if title == "" {
...
func DownloadLatest(d *Drive, stdout bool, format string, force bool) error {
...
func Download(d *Drive, fileId string, stdout, deleteAfterDownload bool, format string, force bool) error {
	// Get file info
	info, err := d.Files.Get(fileId).Do()
	if err != nil {
		return fmt.Errorf("An error occurred: %v\n", err)
	...
	downloadUrl, extension, err := gutil.InternalDownloadUrlAndExtension(info, format)
	if err != nil {
		return err
	}
	...
	// Clode body on function exit
	defer res.Body.Close()

	// Write file content to sdtout
	if stdout {
	...
	if !force && gutil.FileExists(fileName) {
	...
	fmt.Printf("Downloaded '%s' at %s, total %s\n", fileName, getRate(bytes), gutil.FileSizeFormat(bytes, false))
...
func Delete(d *Drive, fileId string) error {
	info, err := d.Files.Get(fileId).Do()
	if err != nil {
		return fmt.Errorf("An error occurred: %v\n", err)
	}
...
func Share(d *Drive, fileId string) error {
	info, err := d.Files.Get(fileId).Do()
	if err != nil {
		return fmt.Errorf("An error occurred: %v\n", err)
	}
	...
	fmt.Printf("File '%s' is now readable by everyone @ %s\n", info.Title, gutil.PreviewUrl(fileId))
	return nil
}

// Removes the 'anyone' permissions -- auth will be required to view/download file
func Unshare(d *Drive, fileId string) error {
	info, err := d.Files.Get(fileId).Do()
	if err != nil {
		return fmt.Errorf("An error occurred: %v\n", err)
	}
...
func Quota(d *Drive, sizeInBytes bool) error {
	info, err := d.About.Get().Do()
	if err != nil {
		return fmt.Errorf("An error occurred: %v\n", err)
	}

	fmt.Printf("Used: %s\n", gutil.FileSizeFormat(info.QuotaBytesUsed, sizeInBytes))
	fmt.Printf("Free: %s\n", gutil.FileSizeFormat(info.QuotaBytesTotal-info.QuotaBytesUsed, sizeInBytes))
	fmt.Printf("Total: %s\n", gutil.FileSizeFormat(info.QuotaBytesTotal, sizeInBytes))
	return nil
}

func isShared(d *Drive, fileId string) bool {
